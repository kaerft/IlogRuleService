package com.sinosoft.ilogrule.restfulService.client.prpallClient;
...
import com.sinosoft.ilogrule.ilog.dto.prpallDto.ApplicantDto;
import com.sinosoft.ilogrule.ilog.dto.prpallDto.InsuredDto;
import com.sinosoft.ilogrule.ilog.dto.prpallDto.ItemKindDto;
import com.sinosoft.ilogrule.ilog.dto.prpallDto.MainDto;
import com.sinosoft.ilogrule.ilog.dto.prpallDto.OtherDto;
...
import com.sinosoft.ilogrule.ilog.dto.prpallDto.VehicleDto;
import com.sinosoft.ilogrule.ilog.dto.prpallDto.VehicleOwnerDto;
import com.sinosoft.ilogrule.restfulService.client.baseClient.DealHeadInfo;
import com.sun.jersey.api.client.ClientResponse;
import com.sun.jersey.api.client.WebResource.Builder;

public class PrpallClient {


	public UnderwriteResultDto undwrtCarRule(UndwrtInputDto requestBody) throws Exception{
		// 是否成功标识
		String Status = "";
		// 错误类型
		...
		UnderwriteResultDto responseBody = null;
		try{
			//获取访问地址(数据库配置)
			String url = "http://10.21.63.4:7001/IlogRuleService/rest/PrpallRuleRestfulService/prpallCarRule";	
			//String url = "http://10.100.17.9:7011/IlogRuleService/rest/PrpallRuleRestfulService/prpallCarRule";	

			System.out.println("调用规则引擎查询接口URL==="+url);
			//获取连接超时时间			
			String limitQueryName = "20000";
			//获取报文头部信息组织
			String SVCNAME = "PrpallRuleRestfulService";
			String ORISYS = "prpall";
			String MTDNAME = "prpallCarRule";
			// 组织公共头信息和连接超时时间
			...
			// 获取请求体信息
			responseBody = response.getEntity(UnderwriteResultDto.class);
			// 处理返回头信息
	...
	public UndwrtInputDto organizeObject() {
		// 基本信息
		...
		mainXom.setRenewalFlag("是");//非空，码表 目前没提供
		// 承保机构
		mainXom.setComCode("61");//非空
		...
		// 险种代码
		mainXom.setRiskCode("2212");//非空
		// 保险起期
		mainXom.setStartDate("2018-07-05");//"2017-12-09" 非空
		// 保险止期
		mainXom.setEndDate("2018-07-01");//"2018-12-08" 非空
		// 投保日期
		...
		// 是否即时生效
		mainXom.setIsTimelyEffective("1");//非空  0 --否 ，1-- 是

		// 特约内容
//		mainXom.setEngageContext("特别约定");//可为空 多条中间用 | 符号分开
//		//政策类型
...
//		mainXom.setStopTimes("");//可为空  
		mainXom.setHandler_Code("88000157");
		mainXom.setHandlerName("880");
		
		// 投保人信息
		ApplicantDto applicantXom = new ApplicantDto();
		// 客户渠道
		applicantXom.setCustomerChannel_Code("3");//非空 码表
		applicantXom.setCustomerChannel("加油站");//非空 码表
		//投保人名称
		applicantXom.setAppliCode("21212");//非空
		applicantXom.setAppliName("货物");//非空
		//投保人类型
		applicantXom.setInsuredNature_Code("0");//非空 码表
		applicantXom.setInsuredNature("自然人");//非空 码表
		//投保人客户行业
...
//		applicantXom.setLinkerMobile("");//可为空
		...
		insuredXom.setInsuredName("张三");//非空
		//被保人类型
		insuredXom.setInsuredNature_Code("0");//非空 码表
		insuredXom.setInsuredNature("非自然人");//非空 码表
		//被保人客户行业
		...
		//被保人证件号码
		insuredXom.setIdentifyNumber("212121111");//非空
//		//年龄
...
//		insuredXom.setLinkerMobile("");//可为空
		//是否黑名单客户
		insuredXom.setIsBlacklist("否");//非空
		...
		vehicleXom.setVehicleBrandName("1");//非空
		// 车型代码
		...
		vehicleXom.setUsenature_Name("特种车");//非空 码表
		// 营业性质
		vehicleXom.setBusinessClassCode("35112001");//非空 码表
		vehicleXom.setBusinessClassName("营业");//非空 码表
		// 核定载客数(座位数)
		vehicleXom.setSeatCount("5");//非空
		// 车辆类型
		vehicleXom.setVehiclesType_Code("345002009");//非空 码表
		vehicleXom.setVehiclesType("二吨至五吨");//非空 码表
		// 车辆过户标识
		vehicleXom.setTransferFlag_Code("1");//非空 码表
		vehicleXom.setTransferFlag("是");//非空 码表
		// 是否上牌
		vehicleXom.setIsOneCard("是");//非空
		// 车船税标志
		//vehicleXom.setHfName_Code("");//可为空 码表 交强有值
		//vehicleXom.setHfName("");//可为空 码表 交强有值
		// 车牌号
		vehicleXom.setLicenseNo("沪FQ0201");//非空
		// 纳税类型
		vehicleXom.setTaxType("N");//非空 纳税，应该有码表，目前没有
		// 新车发票购买日期
		vehicleXom.setBuyDate("2017-12-07");//"2007-12-07" 非空
		// 发证日期
		vehicleXom.setCertificateDate("2017-12-07");//"2007-12-07" 非空
		// 初登日期
		vehicleXom.setEnrollDate("2018-07-01");//"2007-12-07" 非空
		// 行驶里程
		...
		vehicleXom.setFuelType("汽油");//非空 码表
		// 是否改装
		vehicleXom.setIsRefit_Code("0");//非空 码表
		vehicleXom.setIsRefit("否");//非空 码表
		// 车型编码
		vehicleXom.setCarCode("1000");//非空
		
		// 新车购置价
		vehicleXom.setPurchasePrice("120000");//非空
		// 车龄
		vehicleXom.setCarAge("5");//非空
		// 车辆发动机号
		...
		vehicleXom.setIsNewCar("未上牌车辆");//非空 码表
		// 特殊车辆系数
		vehicleXom.setSspecialVehicle("1.0");//非空
		//协商实际价值浮动比例
		vehicleXom.setActualFloatRatioValue("0.2");
		// 协商实际价值
		//vehicleXom.setActualValue(0.2);//可为空，后续有改动
		// 是否节能减排车
		...
		// 车主名称
		vehicleOwnerXom.setOwnerName("张三");//非空
		// 车主类型
		...
		// 险别保额
		//itemKindXom.setAmount(100000);//可为空
		// 上年投保是否出险
		...
		itemKindXom1.setItemKindName("车损险");//非空 码表			
		// 险别保额
		itemKindXom1.setAmount("100000");//可为空
		// 上年投保是否出险
		itemKindXom1.setLastAccient("是");//非空 例如：上年投保车损险
		// 是否国产玻璃
		//itemKindXom1.setIsHomemadeGlass("是");//可为空
		// 每座赔偿限额
		//itemKindXom1.setEveryLimitFee(10000);//可为空		
		// 标准保费
		...
		itemKindXom2.setItemKindName("车损险");//非空 码表			
		// 险别保额
		//itemKindXom2.setAmount("1000000");//可为空
		// 上年投保是否出险
		itemKindXom2.setLastAccient("是");//非空 例如：上年投保车损险
		// 是否国产玻璃
		//itemKindXom2.setIsHomemadeGlass("是");//可为空
		// 每座赔偿限额
		//itemKindXom2.setEveryLimitFee(10000);//可为空		
		// 标准保费
		...
		// 上年赔款总金额
//		otherXom.setLastClaimAmount("1000.00");//可为空
//		// 上年是否有赔案记录
//		otherXom.setLasClaimFlag("否");//可为空
//		// 出险次数
...
//		// 上年投保险止期
		otherXom.setLastEndDate("2017-12-20");//可为空	
//		//无赔优待系数（无赔优NCD系数）
//		otherXom.setClaimadjustValue("1.0");//可为空
		//验车照片数量
		otherXom.setCheckCarPhotosCount("5");//非空
		//本次纳税年度
		otherXom.setThisTaxYear("2018");//非空
		//是否需要验车
		otherXom.setIsValidateCar("是");
		
		UndwrtInputDto undwrtInputDto = new UndwrtInputDto();
